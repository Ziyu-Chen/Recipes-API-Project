...
describe("#Server", () => {
  ...
  describe("deleteRecipe", () => {
    beforeEach(() => {
      ...
      request.post("/graphql").send(body);
    });
    it("should delete the recipe with the given name or id", async () => {
      const body = {
        query: `
        mutation{
					deleteRecipe(name: "Pork Tenderloin with Apples and Onions") {
						name
					}
				}`
      };
      const res = await request.post("/graphql").send(body);
      const lastItem = JSON.parse(res.text).data.deleteRecipe.pop();
      lastItem.should.not.include({
        name: "Pork Tenderloin with Apples and Onions"
      });
    });
  });
  describe("updateRecipe", () => {
    it("should update the recipe with the given name or id", async () => {
      const body = {
        query: `
        mutation{
					updateRecipe(name: "Whole Wheat Molasses Flat Bread" input: {name: "Whole Wheat Molasses Flat Bread"
					description: "his is an easy, tasty main dish for the family on a weeknight. While the pork is in the oven, it's simple to steam a veggie side and prepare a quick salad"
					categories: ["Meat and Poultry", "Pork", "Tenderloin"]
					rating: 5.0
					ingredients: ["2 (1 1/2 pound) pork tenderloins",
      ...
      const res = await request.post("/graphql").send(body);
      JSON.parse(res.text).data.updateRecipe.should.include({
